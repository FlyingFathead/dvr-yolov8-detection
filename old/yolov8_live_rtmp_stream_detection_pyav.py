#!/usr/bin/env python3
#
# yolov8_live_rtmp_stream_detection_pyav.py
# Sept 14 // 2024
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
# Version number
#
# NOTE: this is the `pyav` version
# used only for compatibility tests
# requires: `pip install av`
# (it's recommended to use the original script)
...
# Frame capture thread using PyAV
    ...
    if use_webcam:
        ...
        while retries < MAX_RETRIES and not stop_event.is_set():
            try:
                ...
                container = av.open(stream_url, timeout=TIMEOUT * 1000000)  # timeout in microseconds
                main_logger.info("Successfully opened video stream.")
                break
            except av.AVError as e:
                main_logger.error(f"Unable to open video stream: {stream_url}. Error: {e}. Retrying in {RETRY_DELAY} seconds...")
    ...
    while not stop_event.is_set():
        if use_webcam:
            ...
            try:
                ...
                main_logger.error(f"Error decoding stream: {e}. Attempting to reconnect...")
                container = None
                retries = 0
                while retries < MAX_RETRIES and not stop_event.is_set():
                    try:
                        main_logger.info(f"Re-attempting to open video stream: {stream_url}")
                        container = av.open(stream_url, timeout=TIMEOUT * 1000000)  # timeout in microseconds
                        main_logger.info("Successfully reopened video stream.")
                        break
                    except av.AVError as e:
                        main_logger.error(f"Reconnection failed: {e}. Retrying in {RETRY_DELAY} seconds...")
                        time.sleep(RETRY_DELAY)
                        retries += 1
                if container is None:
                    main_logger.error(f"Failed to reconnect to video stream after {MAX_RETRIES} attempts.")
                    break

    if use_webcam:
        cap.release()
        main_logger.info("Webcam released.")
    else:
        if container:
            container.close()
            main_logger.info("Video stream container closed.")

# Frame processing thread remains largely the same
