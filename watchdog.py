...
def run_watchdog():
    ...
    if ENABLE_HARD_RESET: logging.warning(f"Hard reset is ENABLED for both freeze and disconnect events. Command: '{HARD_RESET_COMMAND}'")
    ...
    while True:
        try:
            ...
            if cap is None or not cap.isOpened():
                ...
                # This block now runs on ANY connection failure.
                if ENABLE_DISCONNECT_ALERTS:
                    if connection_fail_start_time is None:
                        # If the timer isn't running, start it. This now happens on the very first failure.
                        logging.warning("Failed to connect to stream. Starting disconnect timer...")
                        connection_fail_start_time = time.time()
                    
                    # Check if the disconnect duration has passed the threshold
                    if (time.time() - connection_fail_start_time) > DISCONNECT_THRESHOLD_SECONDS:
                        ...
                        if time_to_alert:
                            last_disconnect_alert_time = time.time() # Mark time to handle repeats
            ...
            if connection_fail_start_time is not None:
                # If the timer was running, it means we just recovered.
                ...
                if last_frame_signature is not None:
                    ...
                    if diff_sum < FREEZE_SENSITIVITY_THRESHOLD:
                        ...
                        if (time.time() - freeze_start_time) > FREEZE_DURATION_SECONDS:
                            ...
                            if time_to_alert:
                                last_freeze_alert_time = time.time() # Mark time to handle repeats
                                if ENABLE_HARD_RESET:
